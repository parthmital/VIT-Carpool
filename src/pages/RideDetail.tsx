...
export default function RideDetail() {
  ...
  const { getRideById, joinRide, hasJoinedRide } = useRides();
  const { user } = useAuth();
  const [justJoined, setJustJoined] = useState(false);

  const ride = getRideById(id || '');
  const hasJoined = hasJoinedRide(id || '') || justJoined;
  ...
  const openWhatsApp = () => {
    const message = encodeURIComponent(`Hi! I joined your ride from ${ride.source} to ${ride.destination} on ${formattedDate}.`);
    window.open(`https://wa.me/${ride.creatorWhatsApp}?text=${message}`, '_blank');
  };
  ...
  return (
    <AppLayout>
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <Card className="animate-fade-in">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wide">Posted by</p>
                <p className="font-medium text-foreground">{ride.creatorName}</p>
        ...
        {/* WhatsApp Contact Card - Only shown after joining */}
        {hasJoined && !isCreator && (
